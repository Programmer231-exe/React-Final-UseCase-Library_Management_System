...
const UserBookTable = props => {
  ...
  if (role === "user") {
    ...
    if (category === "status") {
      myrows = rows.filter(
        p => p.status === "Reserved" || p.status === "Checked Out"
      );
    }
  ...
  if (searchValue !== undefined) {
    switch (searchCategory) {
      ...
      case "title": {
        myrows = myrows.filter(p =>
          p.booktitle.toLowerCase().includes(searchValue.toLowerCase())
        );
        break;
      }
      case "subject": {
        let subjectBooks = bookdescription
          .filter(p =>
            p.category.split(",").some(p => {
              console.log(p);
      ...
      case "userid":
        myrows = myrows.filter(p =>
          p.userid.toLowerCase().includes(searchValue.toLowerCase())
        );
        break;
  ...
  const handleDescription = data => {
    ...
    setDescription(
      bookdescription.find(
        (currentValue, index, arr) => currentValue.id === data.id
      )
    );
  };
  ...
  const handleReserve = row => {
    ...
    setConfirmation({
      confirmwindow: true,
      type: "Reserved",
      confirmmsg: "Are you Sure to Add this book to your reserved list?"
    });
  };
  ...
  const handleCheckout = row => {
    ...
    setConfirmation({
      confirmwindow: true,
      type: "Checked Out",
      confirmmsg: "Are you Sure to check out  this book?"
    });
  };
  ...
  const handleRemove = row => {
    ...
    setConfirmation({
      ...
      userid: "",
      type: "Available",
      confirmmsg: "Are you Sure to remove  this book?"
    });
  };
  ...
  return (
    <React.Fragment>
      {(rowsPerPage > 0
        ...
        return (
          <StyledTableRow key={row.id}>
            ...
            {category !== "users" && (
              <TableCell align="left" onClick={() => handleDescription(row)}>
                {row.booktitle}
              </TableCell>
            )}
            {category === "Available" && (
            ...
            {((role === "user" && category !== "Checked Out") ||
              (role === "librarian" && category === "allbooks")) && (
              <TableCell align="center">{row.position}</TableCell>
            )}
            {(category === "Available" || role === "librarian") &&
              category !== "users" && (
                <TableCell align="center">
                  {row.status === "Checked Out" && (
                    <Alert severity="error">{row.status}</Alert>
                  )}
                  {row.status === "Reserved" && (
            ...
            {role === "user" && (
              <TableCell align="center">
                {category === "Available" && (
                  <>
                    <IconButton onClick={() => handleReserve(row)}>
                      <FavoriteBorderIcon />
                    </IconButton>
            ...
            {category !== "users" && role === "librarian" && (
              <TableCell
                align="center"
                onClick={() => handelUserDescription(row)}
              >
                {row.userid}
              </TableCell>
            )}
            {category === "users" && (
              <>
                <TableCell
                  align="center"
                  onClick={() => handelUserDescription({ userid: row.id })}
                >
      ...
      <UserDescription
        opendescription={openuserdescription}
        description={description}
        setOpenDescription={setOpenUserDescription}
      />
      <BookDescription
      ...
      />
      <ConfirmationCheck
        open={confirm.confirmwindow}
        confirm={confirm}
        handleClose={handleClose}
        reserveorCheckoutBook={reserveorCheckoutBook}
      />
    </React.Fragment>
  );
};
