...
export const login = (user, callback, errorcallback) => {
  let loginObj = user;
  let loginUrl = `${LOGIN_URL}/${loginObj.username}.json`;
  try {
    ...
    fetch(loginUrl)
      ...
      .then(
        data => successlogin(data, loginObj, callback),
        err => {
          console.log(err);
          errorcallback({ error: true, errormessage: "Username not existed" });
        }
      )
      .catch(err =>
        errorcallback({
...
export const GetData = (dataType, callback, id) => {
  switch (dataType) {
    case LIBRARIAN:
      sendRequest("GET", `${LIBRARIAN_URL}/${id}.json`, callback, null);
      break;
    case USERS:
      if (id === "") {
        sendRequest("GET", `${USERS_URL}.json`, callback, null);
      } else {
        sendRequest("GET", `${USERS_URL}/${id}.json`, callback, null);
      }
      break;
    case ADDRESS:
      sendRequest("GET", `${ADDRESS_URL}.json`, callback, null);
      break;
    case BOOKSTORE:
      sendRequest("GET", `${BOOKSTORE_URL}.json`, callback, null);
      break;
    case BOOKDESCRIPTION:
      sendRequest("GET", `${BOOKDESCRIPTION_URL}.json`, callback, null);
      break;
